...
import we.lcx.admaker.entity.ad.AdFlight;
import we.lcx.admaker.entity.ad.AdPackage;
import we.lcx.admaker.entity.ad.AdUnit;
import we.lcx.admaker.entity.ad.QueryFlight;
...
public class Basic {
    ...
    private void initPackages() {
        ...
        TaskResult var1 = HttpExecutor.doRequest(Task.post(URL + URLs.YUNYING_URL + URLs.MAISUI_PACKAGES).cookie(COOKIE).param(Entity.of(Params.MAISUI_PACKAGES)));
        var1.valid("获取广告版位失败");
    ...
    public Map<String, String> queryFlight(String keyword) {
        Map<String, String> var0 = new HashMap<>();
        TaskResult var1 = HttpExecutor.doRequest(Task.post(URL + URLs.YUNYING_LIST)
        ...
        for (QueryFlight var3 : var2) {
            if ("E7D5508C".equals(var3.getMediaCode()))
                var0.put(var3.getName(), var3.getId() + "_" + var3.getAdType());
        }
        return var0;
    }
    ...
    public Ad getAdFlight(Integer id) {
        ...
        TaskResult var1 = HttpExecutor.doRequest(Task.post(URL + URLs.YUNYING_QUERY)
        ...
        HttpExecutor.doRequest(Task.post(URL + URLs.YUNYING_CREATE)
                .cookie(COOKIE).param(Entity.of(Params.YUNYING_CREATE)
                        .put("name", Settings.PREFIX_NAME + "_" + id)
